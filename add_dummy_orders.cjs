...
const statuses = ['접수', '공장발주', '출고대기', '출고완료', '보류'];
...
async function run() {
  ...
  if (customers.length === 0) {
    // Fallback default customers if none exists in db
    customers = [
  ...
  for (const status of statuses) {
    const statusEng = status === '접수' ? 'rec' : status === '공장발주' ? 'work' : status === '출고대기' ? 'rel' : status === '출고완료' ? 'comp' : 'hold';
    console.log(`Generating 50 dummy orders for status: ${status} (${statusEng})...`);
    
    for (let i = 1; i <= 50; i++) {
      const customer = customers[Math.floor(Math.random() * customers.length)];
      const orderDate = new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
      const orderId = `ORD-TEST-${statusEng}-${String(i).padStart(3, '0')}`;
        ...
        items: [
          {
            ...
            note: '테스트용 자동 생성 더미 데이터',
            release_date: orderDate,
            calculated_price: calculatedPrice,
            payment_status: status === '출고완료' ? '결제완료' : '결제전',
            step_weights: status === '접수' ? {} : {
              step1: { before: 3.5, after: 2.8 },
              step2: { before: 2.8, after: 2.0 },
              step3: { before: 2.0, after: 2.0 }
            },
            actual_weight_g: status === '접수' ? null : parseFloat((Math.random() * 10 + 2).toFixed(2))
          }
        ],
  ...
  console.log(`Committing ${totalAdded} dummy orders to Firestore...`);
  await batch.commit();
  console.log('Successfully added all dummy orders.');
}
